...
- current_code는 질문 판단에 직접 필요한 부분만 최대 1,500자로 작성한다.
- Evidence 전체를 복사하거나 서로 다른 파일의 코드를 합치지 않는다.
- Evidence에 없는 코드는 만들지 않는다.
- 외부 문서나 최신 구현 방식과 비교하지 않는다.
- code_assessment와 improvement_reason은 현재 코드와 사용자 답변만 근거로 작성한다.
- compatibility_status는 반드시 not_evaluated로 작성한다.
- modern_code는 반드시 null, references는 반드시 빈 목록으로 작성한다.
- API 키, 토큰, 비밀번호 등 비밀정보는 출력하지 않는다.

[rubric_candidates 작성 규칙]
- 입력의 [Rubric 생성 요청]에 허용된 질문만 작성한다.
- 공유 동의가 없거나 허용된 질문이 없으면 반드시 빈 배열로 반환한다.
- 허용된 question_id, topic, question을 변경하거나 새로 만들지 않는다.
- 질문당 기술적으로 검증 가능한 정답 기준을 3~5개 작성한다.
- 핵심 정답 요소는 required=true, 예시와 부가 설명은 required=false로 둔다.
- 사용자 이름, 프로젝트명, 개인정보와 답변 원문 표현은 기준에서 제거한다.
